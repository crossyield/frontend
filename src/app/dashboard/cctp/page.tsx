...
const tokenMessengerAbi = require("./cctps/TokenMessenger.json");
const messageAbi = require("./cctps/Message.json");
const usdcAbi = require("./Usdc.json");
const messageTransmitterAbi = require("./cctps/MessageTransmitter.json");
...
const main = async () => {
  ...
  const OP_TOKEN_MESSENGER_CONTRACT_ADDRESS =
  ...
  const approveTxGas = await usdcEthContract.methods
    .approve(OP_TOKEN_MESSENGER_CONTRACT_ADDRESS, amount)
    .estimateGas();
  ...
  const log = transactionReceipt.logs.find(
    (l: { topics: any[] }) => l.topics[0] === eventTopic
  );
