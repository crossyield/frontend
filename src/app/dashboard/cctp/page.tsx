...
const main = async() => {
    ...
    const OP_TOKEN_MESSENGER_CONTRACT_ADDRESS = "0x9f3B8679c73C2Fef8b59B4f3444d4e156fb70AA5";
    const USDC_OP_CONTRACT_ADDRESS = "0x5fd84259d66Cd46123540766Be93DFE6D43130D7";
    const ETH_MESSAGE_CONTRACT_ADDRESS = ""
    const ETH_MESSAGE_TRANSMITTER_CONTRACT_ADDRESS = '0x7865fAfC2db2093669d92c0F33AeEF291086BEFD';

    // initialize contracts using address and ABI
    const ethTokenMessengerContract = new web3.eth.Contract(tokenMessengerAbi, OP_TOKEN_MESSENGER_CONTRACT_ADDRESS, {from: ethSigner.address});
    const usdcEthContract = new web3.eth.Contract(usdcAbi, USDC_OP_CONTRACT_ADDRESS, {from: ethSigner.address});
    const ethMessageContract = new web3.eth.Contract(messageAbi, ETH_MESSAGE_CONTRACT_ADDRESS, {from: ethSigner.address});
    const avaxMessageTransmitterContract = new web3.eth.Contract(messageTransmitterAbi, ETH_MESSAGE_TRANSMITTER_CONTRACT_ADDRESS, {from: avaxSigner.address});
    ...
    const approveTxGas = await usdcEthContract.methods.approve(OP_TOKEN_MESSENGER_CONTRACT_ADDRESS, amount).estimateGas()
    const approveTx = await usdcEthContract.methods.approve(OP_TOKEN_MESSENGER_CONTRACT_ADDRESS, amount).send({gas: approveTxGas})
